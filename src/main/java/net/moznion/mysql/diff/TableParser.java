...
public class TableParser {
  public static void parse(String tableDefinition) {
    ...
    Matcher matcherForTableName =
        Pattern.compile("^\\s*create\\s+table\\s+(\\S+)\\s+\\(\\s*$", Pattern.CASE_INSENSITIVE)
            .matcher(firstLine);
    ...
    while (lineIter.hasNext()) {
      ...
      if (matcherForPrimaryKey.find()) {
        ...
        if (table.hasPrimaryKey()) {
          throw new IllegalArgumentException(new StringBuilder()
              .append("Two primary keys in table ").append(table.getName()).append(": '")
              .append(primaryKey).append("', '").append(table.getPrimaryKey()).append("'")
              .toString());
        }
      ...
      if (matcherForKey.find()) {
        ...
        if (table.hasIndex(key)) {
          throw new IllegalArgumentException(new StringBuilder().append("index '").append(key)
              .append("' duplicated in table '").append(table.getName()).append("'").toString());
        }
      ...
      if (matcherForFullTextIndex.find()) {
        ...
        if (table.hasFullTextIndex(key)) {
          throw new IllegalArgumentException(new StringBuilder().append("FULLTEXT index '")
              .append(key).append("' duplicated in table '").append(table.getName()).append("'")
              .toString());
        }
      ...
      if (matcherForFieldDefinition.find()) {
        ...
        if (table.hasField(fieldName)) {
          throw new IllegalArgumentException(new StringBuilder().append("definition for field '")
              .append(fieldName).append("' duplicated in table '").append(table.getName())
              .append("'").toString());
        }
    ...
    List<String> garbageLines =
        lines.stream().filter(line -> !line.matches("^/\\*!40\\d{3} .*? \\*/;"))
            .filter(line -> !line.matches("^(?:SET |DROP TABLE)")).collect(Collectors.toList());
